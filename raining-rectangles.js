...
function playerClick (evt) {
  ...
  if ( diffPos[0] >= 0 && diffPos[0] < rainingRect.size[0]
    ...
    if(rainingRect.size[0] <= rainingRect.size[1])
      alert("*******bomb********\n<Push the enter and keeping>");
...
function Bombrec(){ //폭탄 직사각형

  var bomb = this;

  var randNums2 = getRandomVector2();

  bomb.size = [ 5, 5];
  bomb.position = [randNums2[0]*(gl.drawingBufferWidth - bomb.size[0]),
    gl.drawingBufferHeight];

  bomb.velocity = 1.0 + 6.0*Math.random();
  bomb.color = black;
  //gl.clearColor(rect.color[0], rect.color[1], rect.color[2], 1.0);
